%Function that generates n vectors on the sphere that solve the Thomson problem (J.J. Thomson 1904).
%The input n should be an integer corresponding to the number of desired vectors on the sphere
%Visual is an optional variable, if any value is asigned to Visual (e.g.,VIsual=1), the code will plot do a 3d representation of the vectors 
%This code is based on another one from Aaron Becke (see below)
%This adaptation was made by Marco Túlio Quintino, https://github.com/mtcq

function v = generateNonParallelAxes(n,Visual)
...
options = optimset('MaxIter', 10000, 'MaxFunEvals', 10000, 'GradObj', 'on','TolFun',1e-8,'Algorithm','trust-region-reflective','Display','off');
